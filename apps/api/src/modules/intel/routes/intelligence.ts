...
router.get('/intelligence/ioc/:value', async (c) => {
    ...
    // 5. Look up pulse context from PostgreSQL (IOC → Pulse → Actor chain)
    let pulseContext: {
        pulseName: string;
    ...
    try {
        ...
        if (iocRows[0]?.pulseId) {
            const pulseRows = await db.select({
                ...
                .where(eq(pulses.otxId, iocRows[0].pulseId))
                .limit(1);
        ...
        // Pulse context is optional enrichment
    }
