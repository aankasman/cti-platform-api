...
router.get('/iocs/:idOrValue', async (c) => {
    ...
    // Use OpenSearch getById for lookup by ID or value
    ...
    const mappedData = {
        ...item,
        threatType: item.description !== 'unknown' ? item.description : null,  // Map description to threatType if not 'unknown'
        firstSeen: item.createdAt,
        lastSeen: item.updatedAt,
    };
